...
class Converter:
    ...
    def op_concat(self, op):
        ...
        opts.Init(op.BuiltinOptions().Bytes, op.BuiltinOptions().Pos)
        if opts.Axis() not in (3, -1):
            raise SystemExit("only channel-axis concatenation is supported")
        out_idx = op.Outputs(0)
        self.emit(
            {
                "op": "concat",
...
def main():
    ...
    conv.run({idx for (_, idx, _) in heads.values()})
    ...
    spec = {
        ...
        # the model carries its own normalization, so hand it raw 0..255 values
        "input_scale": 255.0,
        "input_bias": 0.0,
        "layers": conv.layers,
        "outputs": {role: conv.emitted[conv.resolve(idx)] for role, (_, idx, _) in heads.items()},
    }
    ...
    for role, (op_index, _, name) in heads.items():
        print(f"  {role:8s} <- op {op_index}: {name[-60:]}")
    if conv.asymmetric:
